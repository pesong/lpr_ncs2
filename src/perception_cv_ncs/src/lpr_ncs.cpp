...
namespace lpr_ncs {
    ...
    void LPR_NCS::init() {
        ...
        nodeHandle_.param("model_path", model_path, std::string("graph"));
        nodeHandle_.param("topics/camera_receive/topic", cameraTopicName, std::string("/camera/image"));
        nodeHandle_.param("topics/camera_result/topic", OutTopicName, std::string("/camera/lpr_out"));
    ...
    void LPR_NCS::plates_infer(cv::Mat image_in){
        ...
        show_lpr_result(image_in, results, det_threshold, out_frame, model_path);
...
/*
    ...
    void LPR_NCS::init_ncs() {
        ...
        GRAPH_FILE_NAME_DET = new char[model_path.length() + 1];
        GRAPH_FILE_NAME_FINE = new char[model_path.length() + 1];
        ...
        strcpy(GRAPH_FILE_NAME_DET, (model_path + "/" + graphModelDet).c_str());
        strcpy(GRAPH_FILE_NAME_FINE, (model_path + "/" + graphModelFine).c_str());
